...
## 头文件规范
### 1. 只包含必要的头文件     
例如如下代码，uart.h就是是一个多余的头文件，这不会导致编译的出错，但我们并不需要使用uart.h中声明的接口，因此这种行为会导致编译效率的降低。如果此时uart.h中还包含了其它文件，那么全部都会在预处理时展开到test.c中。当一个项目的代码量很大时，由于效率地下而多占用的时间就无法忽视了。
``` c
#include <test.h>
#include <stdio.h>
#include <uart.h>
...
### 2. 使#include出现在需要的地方
1.全部include写在.c文件中  
还是上面那个代码，我如果把后面两个include放在test.h文件中，编译也可以通过，但是我们已经知道.h文件的作用相当于模块的使用说明书，如果其它模块要使用test模块时，需要通过#include<test.h>来添加接口函数，此时就会间接的包含了stdio.h，同样的会导编译致速度下降的问题。
### 3. include按顺序引用
例如如下格式（包括空行也是）：
``` c
...
```
### 5. 结构体引用
从下面的代码来看，foo.h和foo.c都需要了解stu_st结构体的内容，也就是stu_st是被foo.c和foo.h同时需要的，因此它必须被放置在foo.h中，否则编译时会出现stu_st未定义的错误，此时将#include放置在.h文件中是必须的！
```c
#ifndef __foo_h__
#define __foo_h__
#include "system.h"
...
### 类型命名
类型名称的每个单词首字母均大写, 不包含下划线。例如：ChassisSpeed
...
## 项目架构
项目总文件下，Cubemx生成的格式保持不变，自己创建的文件全部存于User_File,具体架构为：
